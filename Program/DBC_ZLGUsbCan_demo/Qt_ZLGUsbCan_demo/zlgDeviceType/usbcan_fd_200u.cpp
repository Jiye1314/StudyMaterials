...
//        get_can_canfd_frame(trans_data[i], ch, i + 0x100, is_fd);     //合并发送模式报文
//    }
...
void USBCAN_FD_200U::get_canfd_frame_fd_200u(ZCAN_TransmitFD_Data &canfd_data, canid_t id)
{
    ...
    canfd_data.frame.can_id = MAKE_CAN_ID(id, 0, 0, 0); // CANFD ID + STD/EXT + DATA/RMT
    canfd_data.frame.len = 8;                          // CANFD 数据长度 8
    canfd_data.transmit_type = 0;                      // 正常发送
    canfd_data.frame.flags |= TX_ECHO_FLAG;            // 发送回显
    for (int i = 0; i < 8; ++i) {                       // 填充 CANFD 报文 DATA
